    ...
    if dataset_option == 'pets':
        with loading_placeholder:
            with st.spinner('Dataset is loading...'):
                dataset2 = load_dataset("pcuenq/oxford-pets", split = "train" , streaming = True)
    elif dataset_option == "food101":
        with loading_placeholder:
            with st.spinner('Dataset is loading...'):
                dataset2 = load_dataset("food101", split = "train", streaming=True)
    print("Pozvana fja")
    return dataset2
...
def preprocess_images():
    ...
    dataset2 = load_dataset_for_search()
    #predobrada slika za postupak ekstrakcije visual feature-a
    #my_iterable_dataset = dataset.shuffle(seed=42, buffer_size=100)
    my_iterable_dataset = dataset2.take(number_of_pictures)
    global numpy_array_all2
    numpy_array_all2 = []
    #moramo da dodamo i sliku koju korisnik bira
    if numpy_img is not None:
        numpy_array_all2.append(cv2.cvtColor(numpy_img, cv2.COLOR_BGR2GRAY))
    with loading_placeholder:
        with st.spinner('Converting images to black and white...'):
            for example in my_iterable_dataset:
                numpy_array_all2.append(cv2.cvtColor(np.array(example['image']), cv2.COLOR_BGR2GRAY))
    return numpy_array_all2
...
def search_for_similar_images(no_of_similar_images):
    ...
    for i in idx:
        ...
        if uploaded_custom_dataset:
            ...
            col_pictures.image((numpy_array_all2[i]), caption = cos_sim)
